...
class ChiSquareAnalysis(object):
    ...
    def _get_top_and_bottom_levels(level_names, level_contributions, tops = None, bottoms = None):
        sorted_ = sorted(zip(level_names,level_contributions,list(range(len(level_names)))),reverse=True,key = lambda x: x[1])
        level_names,level_contributions, level_indices = list(zip(*sorted_))
    ...
    def _generate_narratives_card1(self):
        ...
        data_dict['top_levels_percent'] = NarrativesUtils.round_number(old_div(top_dims_contribution*100.0,total))
        ...
        data_dict['best_second_target_percent'] = round(old_div(second_target_contributions[best_second_target_index]*100.0,total),2)
        data_dict['worst_second_target'] = levels[worst_second_target_index]
        data_dict['worst_second_target_percent'] = round(old_div(second_target_contributions[worst_second_target_index]*100.0,total),2)
        ...
        data_dict['best_top_target_percent'] = round(old_div(top_target_contributions[best_top_target_index]*100.0,total),2)
        data_dict['worst_top_target'] = levels[worst_top_target_index]
        data_dict['worst_top_target_percent'] = round(old_div(top_target_contributions[worst_top_target_index]*100.0,total),2)
    ...
    def generate_distribution_card_chart(self, __target, __target_contributions, levels, levels_count, total):
        ...
        bubble_data1 = {}
        bubble_data2 = {}
        bubble_data1['value'] = str(NarrativesUtils.round_number(old_div(max(__target_contributions)*100.0,total),2))+'%'
    ...
    def generate_card1_chart(self):
        ...
        for idx, lvl in enumerate(dim_levels):
            ...
            dict_ = dict(list(zip(data1, data2)))
            data.append(dict_)
