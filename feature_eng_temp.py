    ...
    def split_columns(self):
        ...
        for idx in range(0,len(data_dict['Column_settings'])):
            if data_dict['Column_settings'][idx]['SepSymbols']['value'] == True:
                ...
                for idx in data.index.to_list():                         #print(data[col].str.strip().str.split(),"sepppppp and nnnn")
                    if sep!='':
        ...
        for i in col_list:
          if (data[i].dtype==int) | (data[i].dtype==float):
              if data[i].std()>data[i].mean()/2:
                ...
                if 0.1*data[i].count()<data[i].nunique()<0.9*data[i].count():
...
#         data =data.drop(target,axis=1)
                    ...
                    if ((data[i].dtype==object) | (data[i].nunique()<=5))  & ((data[j].dtype==object)|(data[j].nunique()<=5)):
                        if not j+i in list(data.columns):
                            ...
                            if not j+i in list(data.columns):
                                if data[i].dtype==object:
                                         ...
                                         print("Somewhere in the code: data[j].dtype: ",j, data[j].dtype, (data[j].dtype==object))
                                         ...
                                         if (self.cramers_corrected_stat(data[j],labels1) <0.5) & (self.cramers_corrected_stat(data[j],data[target]) <0.5) & (self.cramers_corrected_stat(labels1,data[target]) <0.5):
                                                ...
                                                try:
                                                    try:
                                                         ...
                                                         data_dict['multiplied_encoded2_series1_2'].append(combined_list3)
                                                    except:
                                                         data[i+j]=data[i]*labels2
                                                         data_dict['multiplied_encoded1_series2_2'].append(combined_list3)

                                                except:
                                                    try:
                                                        data[i+j]=labels1*labels2
                                                        data_dict['multiplied_encoded_both_2'].append(combined_list3)
                                                    except:
                                                        data[i+j]=data[i]*data[j]
                                                        data_dict['multiplied_series_both_2'].append(combined_list3)
                        ...
                        if not j+i in list(data.columns):
                            ...
                            try:
                                ...
                                labels2=data[j]
                            print("data[i], data[j]",i, data[i].dtype, j, data[j].dtype)
                            print("!!!!!!!!int and str!! ","correlation: data[i].corr(data[j])", data[i].corr(data[j]))
                            print("cramers_corrected_stat(labels1,data[target]), ", cramers_corrected_stat(labels1,data[target]))
                            print("cramers_corrected_stat(labels2,data[target]) ",cramers_corrected_stat(labels2,data[target]))
                            if (-0.5<(data[i].corr(data[j]))<0.5) & (cramers_corrected_stat(labels1,data[target]) <0.5) & (cramers_corrected_stat(labels2,data[target]) <0.5):
                                le= sklearn.preprocessing.LabelEncoder()
                                combined_list4=[]
                                ...
                                #try:
                                data[i+j]=data[i]*data[j]
                                data_dict['multiplied_series_both_3'].append(combined_list4)
                                columns_list.append(combined_list4)
                                data_dict2['feature_combiner_classification']=data_dict
        return data
    ...
    def feature_combiner_classification_score(self,data,data_dict):
        ...
        elif 'multiplied_encoded1_series2' in data_dict['feature_combiner_classification'].keys():
        ...
        elif 'multiplied_encoded2_series1' in data_dict['feature_combiner_classification'].keys():
        ...
        elif 'multiplied_series_both' in data_dict['feature_combiner_classification'].keys():
        ...
        elif 'added' in data_dict['feature_combiner_classification'].keys():
    ...
    def remove_duplicate(self,data):
        ...
        for x in range(data.shape[1]):
            ...
            for y in range(x + 1, data.shape[1]):
                ...
                #print(type(otherCol),"hgh",type(col),"hhhhhhhhhhhhhhhhhhhhhhhhhh",otherCol.name,col.name)
                #print(type(otherCol) == type(col))
                #print(otherCol == col)
                try:
                    test_col = (otherCol == col)
                    #print("col.equals(otherCol) ", all(x == True for x in test_col))
