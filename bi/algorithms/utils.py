...
from bi.common import NormalCard, NarrativesTree, HtmlData, C3ChartData, TableData, ModelSummary, PopupData, NormalCard, \
    ParallelCoordinateData, DataBox, WordCloud
...
def bucket_all_measures(df, measure_columns, dimension_columns, target_measure=None):
    if target_measure is None:
        target_measure = []
    df = df.select([col(c).cast('double').alias(c) if c in measure_columns else col(c) for c in
                    list(set(measure_columns + dimension_columns + target_measure))])
...
def bin_column(df, measure_column, get_aggregation=False):
    ...
    min_, max_, mean_, stddev_, total = \
    df.agg(FN.min(measure_column), FN.max(measure_column), FN.mean(measure_column), FN.stddev(measure_column),
           FN.sum(measure_column)).collect()[0]
    df_without_outlier = df.filter(col(measure_column) >= mean_ - 3 * stddev_).filter(
        col(measure_column) <= mean_ + 3 * stddev_)
    diff = (max_ - min_) * 1.0
    ...
    if (get_aggregation):
        ...
        for row in agg_df.collect():
            aggr[row[0]] = {'sum': row[1], 'count': row[2], 'sum_percent': row[1] * 100.0 / total,
                            'count_percent': row[2] * 100.0 / df.count()}
...
def cluster_by_column(df, col_to_cluster, get_aggregation=False):
    ...
    if (get_aggregation):
        ...
        for row in agg_df.collect():
            aggr[row[0]] = {'sum': row[1], 'count': row[2], 'sum_percent': row[1] * 100.0 / total,
                            'count_percent': row[2] * 100.0 / final_df.count()}
    final_df = final_df.select([c for c in final_df.columns if c != 'feature_vector'])
    final_df = final_df.select(
        [col(c) if c != col_to_cluster else col(c).alias(col_to_cluster) for c in final_df.columns])
    # print final_df.show(3)
    if (get_aggregation):
...
def get_model_comparison(collated_summary):
    ...
    algos_dict = {"randomforest": "Random Forest", "xgboost": "XGBoost", "logistic": "Logistic Regression",
                  "svm": "Support Vector Machine", "sparkrandomforest": "Spark ML Random Forest",
...
def get_total_models_classification(collated_summary):
    ...
    output = "<p>mAdvisor has built predictive models using {} algorithms ({}) to predict {} and \
        has come up with the following results:</p>".format(len(algos), ",".join(algorithm_name),
                                                            collated_summary[algos[0]]["targetVariable"])
    return output
...
def create_model_summary_para(modelSummaryClass):
    prediction_split_array = sorted([(k, v) for k, v in modelSummaryClass.get_prediction_split().items()],
    ...
    if binary_class:
        if modelSummaryClass.get_algorithm_name() == 'Random Forest':
            ...
            paragraph = "mAdvisor using {} has predicted around <b> {}% </b> of observations as {} and the remaining <b> {}%</b> as {} with an overall accuracy of <b>{}%</b>. The Random Forest model contains around {} trees and {} rules. The model was able to rightly predict {} observations as {} out of the total {}. ".format(
                modelSummaryClass.get_algorithm_name(), prediction_split_array[0][1], prediction_split_array[0][0],
                prediction_split_array[1][1], prediction_split_array[1][0],
                modelSummaryClass.get_model_accuracy() * 100, modelSummaryClass.get_num_trees(),
                modelSummaryClass.get_num_rules(), confusion_matrix[target_level][target_level], target_level,
                __builtin__.sum(confusion_matrix[x][target_level] for x in confusion_matrix.keys()))
        ...
        #     paragraph = "mAdvisor using {} has predicted around <b> {}% </b> of observations as {} and the remaining <b> {}%</b> as {} with an overall accuracy of <b>{}%</b>. The Random Forest model contains around {} trees and {} rules. The model was able to rightly predict {} observations as {} out of the total {}. ".format(modelSummaryClass.get_algorithm_name(),prediction_split_array[0][1],prediction_split_array[0][0],prediction_split_array[1][1], prediction_split_array[1][0], modelSummaryClass.get_model_accuracy()*100, modelSummaryClass.get_num_trees(), modelSummaryClass.get_num_rules(), confusion_matrix[target_level][target_level], target_level, __builtin__.sum(confusion_matrix[x][target_level] for x in confusion_matrix.keys()))
            ...
            target_level = modelSummaryClass.get_target_level()
            confusion_matrix = dict(modelSummaryClass.get_confusion_matrix())
            paragraph = "Using {}, mAdvisor was able to predict <b> {}% </b> of observations as {} and the remaining <b> {}%</b> as {} with an overall accuracy of <b>{}%</b>. This model was evaluated using {} method. When it comes to predicting {}, <b>{}</b> observations were rightly predicted out of the total {} observations. ".format(
                modelSummaryClass.get_algorithm_name(), prediction_split_array[0][1], prediction_split_array[0][0],
                prediction_split_array[1][1], prediction_split_array[1][0],
                modelSummaryClass.get_model_accuracy() * 100, modelSummaryClass.get_validation_method(), target_level,
                confusion_matrix[target_level][target_level],
                __builtin__.sum(confusion_matrix[x][target_level] for x in confusion_matrix.keys()))
        elif modelSummaryClass.get_algorithm_name() == 'Spark ML Logistic Regression':
            target_level = modelSummaryClass.get_target_level()
            confusion_matrix = dict(modelSummaryClass.get_confusion_matrix())
            paragraph = "mAdvisor was able to predict <b> {}% </b> of observations as {} and the remaining <b> {}%</b> as {} using Spark ML Logistic Regression. The model has an overall accuracy of <b>{}%</b>. The model using Spark ML Logistic Regression was able to accurately predict {} observations as {} out of the total {}. ".format(
                prediction_split_array[0][1], prediction_split_array[0][0], prediction_split_array[1][1],
                prediction_split_array[1][0], modelSummaryClass.get_model_accuracy() * 100,
                confusion_matrix[target_level][target_level], target_level,
                __builtin__.sum(confusion_matrix[target_level][x] for x in confusion_matrix.keys()))

        elif modelSummaryClass.get_algorithm_name() == 'Xgboost':
            target_level = modelSummaryClass.get_target_level()
            confusion_matrix = dict(modelSummaryClass.get_confusion_matrix())
            paragraph = "mAdvisor was able to predict <b> {}% </b> of observations as {} and the remaining <b> {}%</b> as {} using XGBoost. The model has an overall accuracy of <b>{}%</b>. The model using XG Boost was able to accurately predict {} observations as {} out of the total {}. ".format(
                prediction_split_array[0][1], prediction_split_array[0][0], prediction_split_array[1][1],
                prediction_split_array[1][0], modelSummaryClass.get_model_accuracy() * 100,
                confusion_matrix[target_level][target_level], target_level,
                __builtin__.sum(confusion_matrix[x][target_level] for x in confusion_matrix.keys()))
            ...
            target_level = modelSummaryClass.get_target_level()
            confusion_matrix = dict(modelSummaryClass.get_confusion_matrix())
            paragraph = "mAdvisor was able to predict <b> {}% </b> of observations as {} and the remaining <b> {}%</b> as {} using Spark ML Random Forest. The model has an overall accuracy of <b>{}%</b>. The model using Spark ML Random Forest was able to accurately predict {} observations as {} out of the total {}. ".format(
                prediction_split_array[0][1], prediction_split_array[0][0], prediction_split_array[1][1],
                prediction_split_array[1][0], modelSummaryClass.get_model_accuracy() * 100,
                confusion_matrix[target_level][target_level], target_level,
                __builtin__.sum(confusion_matrix[target_level][x] for x in confusion_matrix.keys()))
            ...
            target_level = modelSummaryClass.get_target_level()
            confusion_matrix = dict(modelSummaryClass.get_confusion_matrix())
            paragraph = "mAdvisor was able to predict <b> {}% </b> of observations as {} and the remaining <b> {}%</b> as {} using naivebayes. The model has an overall accuracy of <b>{}%</b>. The model using Naive Bayes was able to accurately predict {} observations as {} out of the total {}. ".format(
                prediction_split_array[0][1], prediction_split_array[0][0], prediction_split_array[1][1],
                prediction_split_array[1][0], modelSummaryClass.get_model_accuracy() * 100,
                confusion_matrix[target_level][target_level], target_level,
                __builtin__.sum(confusion_matrix[x][target_level] for x in confusion_matrix.keys()))
            ...
            target_level = modelSummaryClass.get_target_level()
            confusion_matrix = dict(modelSummaryClass.get_confusion_matrix())
            paragraph = "mAdvisor was able to predict <b> {}% </b> of observations as {} and the remaining <b> {}%</b> as {} using Spark ML Naive Bayes. The model has an overall accuracy of <b>{}%</b>. The model using Spark ML Naive Bayes was able to accurately predict {} observations as {} out of the total {}. ".format(
                prediction_split_array[0][1], prediction_split_array[0][0], prediction_split_array[1][1],
                prediction_split_array[1][0], modelSummaryClass.get_model_accuracy() * 100,
                confusion_matrix[target_level][target_level], target_level,
                __builtin__.sum(confusion_matrix[x][target_level] for x in confusion_matrix.keys()))
            ...
            target_level = modelSummaryClass.get_target_level()
            confusion_matrix = dict(modelSummaryClass.get_confusion_matrix())
            paragraph = "mAdvisor was able to predict <b> {}% </b> of observations as {} and the remaining <b> {}%</b> as {} using Multilayer Perceptron. The model has an overall accuracy of <b>{}%</b>. The model using Multilayer Perceptron was able to accurately predict {} observations as {} out of the total {}. ".format(
                prediction_split_array[0][1], prediction_split_array[0][0], prediction_split_array[1][1],
                prediction_split_array[1][0], modelSummaryClass.get_model_accuracy() * 100,
                confusion_matrix[target_level][target_level], target_level,
                __builtin__.sum(confusion_matrix[x][target_level] for x in confusion_matrix.keys()))
        ...
        if modelSummaryClass.get_algorithm_name() == 'Random Forest':
            ...
            paragraph = "mAdvisor using {} has predicted around <b> {}% </b> of observations as {} with an overall accuracy of <b>{}%</b>. The Random Forest model contains around {} trees and {} rules. The model was able to rightly predict {} observations as {} out of the total {}. ".format(
                modelSummaryClass.get_algorithm_name(), target_level_percentage, target_level,
                modelSummaryClass.get_model_accuracy() * 100, modelSummaryClass.get_num_trees(),
                modelSummaryClass.get_num_rules(), confusion_matrix[target_level][target_level], target_level,
                __builtin__.sum(confusion_matrix[x][target_level] for x in confusion_matrix.keys()))
        elif modelSummaryClass.get_algorithm_name() == 'Spark ML Random Forest':
            ...
            confusion_matrix = dict(modelSummaryClass.get_confusion_matrix())
            target_level_percentage = [x[1] for x in prediction_split_array if x[0] == target_level][0]
            paragraph = "Using {}, mAdvisor was able to predict <b> {}% </b> of observations as {} with an overall accuracy of <b>{}%</b>. This model was evaluated using {} method. When it comes to predicting {}, <b>{}</b> observations were rightly predicted out of the total {} observations. ".format(
                modelSummaryClass.get_algorithm_name(), target_level_percentage, target_level,
                modelSummaryClass.get_model_accuracy() * 100, modelSummaryClass.get_validation_method(), target_level,
                confusion_matrix[target_level][target_level],
                __builtin__.sum(confusion_matrix[x][target_level] for x in confusion_matrix.keys()))
            ...
            confusion_matrix = dict(modelSummaryClass.get_confusion_matrix())
            target_level_percentage = [x[1] for x in prediction_split_array if x[0] == target_level][0]
            paragraph = "mAdvisor was able to predict <b> {}% </b> of observations as {} using XGBoost. The model has an overall accuracy of <b>{}%</b>. The model using XG Boost was able to accurately predict {} observations as {} out of the total {}. ".format(
                target_level_percentage, target_level, modelSummaryClass.get_model_accuracy() * 100,
                confusion_matrix[target_level][target_level], target_level,
                __builtin__.sum(confusion_matrix[x][target_level] for x in confusion_matrix.keys()))
        elif modelSummaryClass.get_algorithm_name() == 'Spark ML Random Forest':
            target_level = modelSummaryClass.get_target_level()
            ...
            confusion_matrix = dict(modelSummaryClass.get_confusion_matrix())
            target_level_percentage = [x[1] for x in prediction_split_array if x[0] == target_level][0]
            paragraph = "mAdvisor was able to predict <b> {}% </b> of observations as {} using Multilayer Perceptron. The model has an overall accuracy of <b>{}%</b>. The model using Multilayer Perceptron was able to accurately predict {} observations as {} out of the total {}. ".format(
                target_level_percentage, target_level, modelSummaryClass.get_model_accuracy() * 100,
                confusion_matrix[target_level][target_level], target_level,
                __builtin__.sum(confusion_matrix[x][target_level] for x in confusion_matrix.keys()))
...
def model_management_to_table(modelSummary):
    values = []
    for head, value in modelSummary.items():
        values.append(value)
...
def collated_model_summary_card(result_setter, prediction_narrative, appType, appid=None):
    ...
    if appType == "CLASSIFICATION":
        ...
        if hyperParameterFlag == True:
            ...
            allAlgorithmTable = [allAlgorithmTable[0]] + sorted(allAlgorithmTable[1:], key=lambda x: x[
                allAlgorithmTableHeaderRow.index("Accuracy")], reverse=True)
        ...
        ## Getting Model Management summaries
        sprfManagementSummary = result_setter.get_spark_random_forest_management_summary()
        rfManagementSummary = result_setter.get_random_forest_management_summary()
        nbManagementSummary = result_setter.get_naive_bayes_management_summary()
        spnbManagementSummary = result_setter.get_spark_naive_bayes_management_summary()
        lrManagementSummary = result_setter.get_logistic_regression_management_summary()
        splrManagementSummary = result_setter.get_spark_logistic_regression_management_summary()
        xgbManagementSummary = result_setter.get_xgboost_management_summary()
        spxgbManagementSummary = result_setter.get_spark_xgboost_management_summary()
        svmManagementSummary = result_setter.get_svm_management_summary()
        spmlpcManagementSummary = result_setter.get_spark_multilayer_perceptron_management_summary()

        model_management_summary = []
        # eval_metric_name = model_dropdowns[0]['evaluationMetricName']
        allSummaryTable = []
        summaryHeader = ['Model ID', 'Project Name', 'Algorithm', 'Status', 'Accuracy',
                         'Runtime', 'Created On', 'Owner', 'Deployment','Action']
        allSummaryTable.append(summaryHeader)

        for obj in [sprfManagementSummary, rfManagementSummary, nbManagementSummary, spnbManagementSummary,
                    lrManagementSummary, splrManagementSummary, xgbManagementSummary, spxgbManagementSummary,
                    svmManagementSummary, spmlpcManagementSummary]:

            if obj != None:
                row = []
                for key in summaryHeader:
                    row.append(obj[key])
                allSummaryTable.append(row)

        summaryHtml = HtmlData(data="<h3>Summary</h3>")
        modelManagementCard = NormalCard(name="Model Management Summary Card", slug="Management Summary")
        modelManagementSummaryTable = TableData({'tableType': 'normal', 'tableData': allSummaryTable})
        modelManagementCard.set_card_data([summaryHtml, modelManagementSummaryTable])
        modelManagementCard.set_card_width(100)
        modelManagementCard = json.loads(CommonUtils.convert_python_object_to_json(modelManagementCard))
        model_management_summary.append(modelManagementCard)
        ...
        if hyperParameterFlag == True:
            algoSummaryCard = NormalCard(name="Top Performing Models", slug="FIRSTCARD")
            allAlgorithmTable = [allAlgorithmTable[0]] + sorted(allAlgorithmTable[1:], key=lambda x: x[
                allAlgorithmTableHeaderRow.index("R-Squared")], reverse=True)
            totalModels = len(allAlgorithmTable) - 1
            allAlgorithmTable = allAlgorithmTable[:GLOBALSETTINGS.MAX_NUMBER_OF_MODELS_IN_SUMMARY + 1]
            allAlgorithmTableModified = [allAlgorithmTable[0]]
...
def stock_sense_overview_card(data_dict_overall):
    ...
    try:
        articlesByConceptCardData = {k: round(float(v) * 100 / valuesTotal, 2) for k, v in
                                     articlesByConceptCardData.items()}
...
def aggregate_concept_stats(conceptDictArray):
    ...
    for idx, val in enumerate(concepts):
        ...
        if articlesDict[val] != 0:
            obj = {"concept": val, "articles": articlesDict[val],
                   "avgSentiment": round(sentimentDict[val] / articlesDict[val], 2)}
        else:
