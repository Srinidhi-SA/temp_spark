...
"""TreeFeatureSelectionTechnique class libraries"""

from sklearn.ensemble import ExtraTreesClassifier

from sklearn.ensemble import ExtraTreesRegressor

""" LinearFeatureSelectionTechnique class libraries """
...
class LinearFeatureSelectionTechnique(object):
    ...
    """one way anova or two way anova depends on categorical columns(categorical vs continuous)"""
    ...
    """f_regression(continuous vs continuous)"""
    ...
    def chi2(self):
        ...
        for idx, col_name in enumerate(self.cat_col):
            chi2, p_value, dof, expected = chi2_contingency(
                pd.crosstab(self.Dataframe[col_name], self.Dataframe[self.target]))

            p_values.append([col_name, round(p_value, 3)])

        return dict(p_values)

    """f_classifier(continuous vs categorical)"""
...
class TreeFeatureSelectionTechnique(object):
...
class Utils(object):
    ...
    """ Sklearn one_hot_encoder drop first """

    def one_hot_encoder(self, X, col):
    ...
    def dummified_columns(self, data_dict, Dataframe):

        data_dict['dummified_columns'] = {}

        data_dict['dummified_columns'] = {'linear': {}, 'tree': {}}

        if data_dict['dummy']['linear']:

            for i in data_dict['dummy']['linear']:
                ...
                data_dict['dummified_columns']['linear'][i] = labels

        if data_dict['dummy']['tree']:

            for i in data_dict['dummy']['tree']:
                labels = list(Dataframe[i].value_counts().index)
                ...
                data_dict['dummified_columns']['tree'][i] = labels
...
class FeatureSelection(object):
    ...
    def empty_feature(self, linear_col, key):
        ...
        if dum_col_l != []:
            ...
            for col in dum_col_l:
                X = self.utils.one_hot_encoder(X, col)
    ...
    def linear_run(self, cat_col, num_col, col):
        ...
        linear_fea = LinearFeatureSelectionTechnique(self.Dataframe, self.target, cat_col, num_col)
    ...
    def tree_run(self, cat_col, col):
        ...
        tree_fea = TreeFeatureSelectionTechnique(X, Y, cat_col, self.Dataframe)
    ...
    def run(self):
        ...
        self.info['tree_features'], tree_col_en = self.tree_run(cat_col, col)
        ...
        """One click dictionary is updating """

        self.data_dict['pass_2'] = self.pass_2

        self.data_dict.update(self.info)

        self.data_dict = self.utils.dummified_columns(self.data_dict, self.Dataframe)
