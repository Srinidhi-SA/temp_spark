...
class Data_Validation:
    ...
    ## Dataset Column name Normalisation and Validation :
...
#
        ...
        if re.search(r'[\W_]+',column):
            ...
            re_column = re.sub(r'[^a-zA-Z0-9]+$', '',re_column)

            re_column = re.sub("^[^A-Za-z0-9]*", '', re_column)
        ...
        if re_column != self.target: #or self.df[re_column].dtype == 'O':     ## Creating issue by keeping feature engineered cols
...
#                print("dropped: ",re_column, column)
...
#        print("target_val.nunique(): ",target_val.nunique())
    ...
    def run(self):
        ...
        if(type(self.dataframe) == str):
            ...
            self.df = pd.read_csv(self.dataframe,encoding=self.find_encoding(),error_bad_lines=False,sep=",",na_values=na_values)
...
#        print("Target Fitness: ",self.data_dict["target_fitness_check"])
