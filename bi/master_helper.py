        ...
        for obj in algosToRun:
            ...
            if  obj.get_algorithm_slug() == GLOBALSETTINGS.MODEL_SLUG_MAPPING["TensorFlow"]:
                try:
                    ...
                    CommonUtils.save_error_messages(errorURL,APP_NAME,e,ignore=ignoreMsg)
            if  obj.get_algorithm_slug() == GLOBALSETTINGS.MODEL_SLUG_MAPPING["PyTorch"]:
                try:
                    st = time.time()
                    ...
                    nnptc_obj.Train()
                    print("PyTorch trained in ", time.time() - st,  " seconds.")
                except Exception as e:
                    result_setter.set_nnptc_fail_card({"Algorithm_Name": "PyTorch", "success": "False"})
                    CommonUtils.print_errors_and_store_traceback(LOGGER, "PyTorch", e)
                    CommonUtils.save_error_messages(errorURL, APP_NAME, e, ignore=ignoreMsg)
        ...
        for obj in algosToRun:
            ...
            if obj.get_algorithm_slug() == GLOBALSETTINGS.MODEL_SLUG_MAPPING["PyTorch"]:
                try:
                    ...
                    nnptr_obj.Train()
                    print("PyTorch-R trained in ", time.time() - st,  " seconds.")
                except Exception as e:
                    result_setter.set_nnptr_fail_card({"Algorithm_Name":"PyTorch","Success":"False"})
                    CommonUtils.print_errors_and_store_traceback(LOGGER,"PyTorch",e)
                    CommonUtils.save_error_messages(errorURL,APP_NAME,e,ignore=ignoreMsg)
    ...
    if app_type == "CLASSIFICATION":
        ...
        if "randomforest" in selected_model_for_prediction:
            ...
            try:
                ...
                CommonUtils.save_error_messages(errorURL,APP_NAME,e,ignore=ignoreMsg)
            print("Scoring Done in ", time.time() - st,  " seconds.")
        elif "PyTorch" in selected_model_for_prediction:
    ...
    if app_type == "CLASSIFICATION":
        ...
        if "generalizedlinearregression" in  selected_model_for_prediction:
            ...
            try:
                ...
                CommonUtils.save_error_messages(errorURL,APP_NAME,e,ignore=ignoreMsg)
            print("Scoring Done in ", time.time() - st,  " seconds.")
        if "PyTorch" in  selected_model_for_prediction:
