...
class SklearnGridSearchResult:
    def __init__(self, resultDict={}, estimator=None, x_train=None, x_test=None, y_train=None, y_test=None,
                 appType=None, modelFilepath=None, levels=None, posLabel=None, evaluationMetricDict=None):
    ...
    def train_and_save_models(self):
        ...
        for idx, paramsObj in enumerate(self.resultDf["params"]):
            ...
            row.update(algoEvaluationMetrics)
            paramsObj = dict(
                [(k, str(v)) if (v == None) | (v in [True, False]) else (k, v) for k, v in paramsObj.items()])
            row.update(paramsObj)
            tableOutput.append(row)
        if self.appType == "REGRESSION":
