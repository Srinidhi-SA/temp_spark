...
class ResultSetter(object):
    ...
    def get_all_classification_cards(self):
        map_dict={'Naive Bayes':self.nbcards, 'Logistic Regression':self.lrcards, 'Neural Network':self.nncards,'XGBoost':self.xgbcards, 'Random Forest':self.rfcards,'TensorFlow':self.tfcards, "PyTorch":self.nnptccards}
        all_cards=[]
    ...
    def get_all_regression_cards(self):
        ...
        map_dict={'Linear Regression':self.linrcards, 'Gradient Boosted Tree Regression':self.gbtrcards, 'Decision Tree Regression':self.dtreercards, 'Random Forest Regression':self.rfrcards,'TensorFlow':self.tfregcards, "PyTorch":self.nnptrcards}
        all_cards=[]
        for i in self.model_order:
            all_cards=all_cards+map_dict[i]
        return all_cards
