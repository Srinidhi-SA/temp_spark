...
class TwoWayAnova:
    ...
    def test_all(self, measure_columns=None, dimension_columns=None, max_num_levels=200):
        ...
        for m in measures:
            ...
            if self._dateFormatDetected and self._trend_on_td_column == False:
                self.initialise_trend_object(m)
            elif len(self._timestamp_columns)>0 and self._trend_on_td_column == True:
                self._primary_date = self._timestamp_columns[0]
                self._dateFormatDetected=True
                self.initialise_trend_object(m,use_timestamp=True)
