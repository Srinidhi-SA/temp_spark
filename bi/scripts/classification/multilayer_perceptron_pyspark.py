...
class MultilayerPerceptronPysparkScript:
    ...
    def Train(self):
        ...
        print "="*100
        print "number of input features =", input_feats
        print "="*100
        ...
        for grid in paramGrid:
            for param in grid.keys():
                print param.name, grid[param]
        ...
        mlpcCards = [json.loads(CommonUtils.convert_python_object_to_json(cardObj)) for cardObj in MLUtils.create_model_summary_cards(self._model_summary)]
        for card in mlpcCards:
            self._prediction_narrative.add_a_card(card)

        self._result_setter.set_model_summary({"sparkperceptron":json.loads(CommonUtils.convert_python_object_to_json(self._model_summary))})
