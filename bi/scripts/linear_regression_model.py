...
class LinearRegressionModelPysparkScript:
    ...
    def Train(self):
        ...
        print "$"*20
        trainingData,validationData = MLUtils.get_training_and_validation_data(indexed,result_column,0.8,appType="regression")
        # OriginalTargetconverter = IndexToString(inputCol="label", outputCol="originalTargetColumn")
        linr = LinearRegression(labelCol=result_column, featuresCol='features',maxIter=10, regParam=0.3, elasticNetParam=0.8)
        fit = linr.fit(trainingData)
        transformed = fit.transform(validationData)
        MLUtils.save_pipeline_or_model(fit,model_filepath)
        metrics2 = RegressionMetrics(transformed.select(["prediction",result_column]).rdd)

        # Squared Error
        print("MSE2 = %s" % metrics2.meanSquaredError)
        print("RMSE2 = %s" % metrics2.rootMeanSquaredError)

        # R-squared
        print("R-squared2 = %s" % metrics2.r2)

        # Mean absolute error
        print("MAE2 = %s" % metrics2.meanAbsoluteError)

        # Explained variance
        print("Explained variance2 = %s" % metrics2.explainedVariance)
